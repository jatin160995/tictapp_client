...
class _ProductGridState extends State<ProductGrid> with AutomaticKeepAliveClientMixin<ProductGrid> {
  ...
  Widget build(BuildContext context) {
   ...
   GridView.count(
      ...
      childAspectRatio: ((cellWidth/2) / ((cellWidth/2) + 80)),
      crossAxisSpacing: 10,
      mainAxisSpacing: 10,
      crossAxisCount: 2,
      controller: controller,
  ...
  Future<void> getProducts() async {
    ...
    try
    {
     ...
     final response = await http.post(
          getProductUrl+"&api_token=" + preferences.getString(api_token),
          body: {"key": apiKey, "page": currentPage.toString(), "category_id" : widget.catInfo['category_id'].toString()}
      );
